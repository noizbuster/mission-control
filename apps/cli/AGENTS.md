...
| CLI package targets | `package.json`, `project.json` | Vite multi-entry build (`index`, `args`, `commands/*`) + `tsc --emitDeclarationOnly`, verbose Vitest, Nx `cli:*` targets. |
...
- Normal prompts can run through the deterministic local provider or the OpenAI-compatible adapter when configured. `$name [args]` loads a discovered skill's `SKILL.md` body as the next user prompt (real skill loading, replacing the old scaffold recorder); the loaded body is inert text and does not call Codex host skills or spawn agents on its own. Skill chat syntax is dollar-prefix only; slash never expands to skill. `!command` runs a shell command and submits the output to the model; `!!command` runs a shell command and displays the output only. Both bash prefixes require workspace trust. A real tool-calling provider is required for loaded skills to drive agentic behavior - the default `local/local-echo` provider does not call tools.
...
- Do NOT edit `dist`; it is generated by `vite build` (JS multi-entry + chunks) and `tsc --emitDeclarationOnly` (`.d.ts`).
