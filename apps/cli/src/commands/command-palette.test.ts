 ...
 * in unit tests: it lives in the TUI package which pulls in the native FFI
 * renderer, and the CLI test environment has no DOM/react test harness. So T8
 * pins the palette's correctness at the CLASSIFICATION layer (slash-mapping.ts):
 * this suite asserts the mapping table COVERS every slashCommandChoices entry and
