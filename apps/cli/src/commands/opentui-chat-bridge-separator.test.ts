...
import { buildSeparatorLine } from '../components/Separator.js';
import { createOpenTuiChatBridgeCore, resolveSeparatorState } from './opentui-chat-bridge.js';
...
describe('resolveSeparatorState', () => {
    it('is running while generating', () => {
        const snapshot = { ...createOpenTuiChatBridgeCore().snapshot, generating: true };
        expect(resolveSeparatorState(snapshot)).toBe('running');
    });

    it('is awaiting_input while an approval is pending', () => {
        const snapshot = { ...createOpenTuiChatBridgeCore().snapshot, approvalActive: true };
        expect(resolveSeparatorState(snapshot)).toBe('awaiting_input');
    });

    it('is awaiting_input while a question is pending', () => {
        const snapshot = { ...createOpenTuiChatBridgeCore().snapshot, questionActive: true };
        expect(resolveSeparatorState(snapshot)).toBe('awaiting_input');
    });

    it('is idle when not generating and no overlay is pending', () => {
        expect(resolveSeparatorState(createOpenTuiChatBridgeCore().snapshot)).toBe('idle');
    });

    it('prefers running over awaiting_input when both are active', () => {
        const snapshot = {
            ...createOpenTuiChatBridgeCore().snapshot,
            generating: true,
            approvalActive: true,
        };
        expect(resolveSeparatorState(snapshot)).toBe('running');
