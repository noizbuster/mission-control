...
describe('interactive chat actions', () => {
    ...
    it('/sessions opens the picker modal in TUI mode and attaches to the selected session', async () => {
        ...
        const selectSessionForAttach = vi.fn(async () => 'session_other');
        const listWorkspaceSessions = vi.fn(async () => [pickerEntry('session_current', 'current')]);
        ...
        expect(switchSession).toHaveBeenCalledWith({ sessionId: 'session_other' });
        expect(output.getOutput()).toContain('Switched to session: session_other');
        expect(result.sessionId).toBe('session_other');
    });

    it('/sessions falls back to text list when the TUI picker is unavailable', async () => {
        const runtime = new AgentRuntime();
        const output = createOutput();
