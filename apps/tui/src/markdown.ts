/**
 * Pure streaming markdown healer + block splitter (no React, no opentui).
 *
 * Extracted from the CLI app so both packages compile during the component
 * move (Todo 4). Consumed by the OpenTUI `Markdown` component (which stays in
 * the CLI app until Todo 4) via `@mission-control/tui/markdown`.
 *
 ...
 *
 * Ported from opencode's `markdown-stream.ts`, adapted to strict TypeScript
 * (`noUncheckedIndexedAccess`, `verbatimModuleSyntax`, `lib: ES2022`):
