...
| Tool advertising + permission model | `docs/tool-permission-model.md` | How node `capabilities`, tool `capabilityClasses`, approval, trust, and skills interact (OpenCode-style expand map). |
...
| Workflow runtime foundations | see **Workflow Foundations** below | Permission rule algebra, `.mc/` persistence, Mission/Run store, drain-lane coordinator v2, context-source registry, continuation runtime, full-parity `task()` tool. |
...
| Executer admission + verdict | `packages/core/src/behavior/executer-plan-admission.ts`, `executer-workflow-graph.ts` | `evaluatePlanAdmission` (pure gate rejecting empty/missing-section/no-todos/unapproved plans); `aggregateFinalVerdict` (all-approve → `APPROVE`, else `REJECT`). |
...
| `aggregateFinalVerdict` | function | `packages/core/src/behavior/executer-workflow-graph.ts` | Pure `'APPROVE'` iff every F1-F4 critic output is `'APPROVE'`, else `'REJECT'` (fail-closed). The executer `final-verification-wave` node aggregates via `verdictStrategy: 'all-approve'`. |
| `evaluatePlanAdmission` | function | `packages/core/src/behavior/executer-plan-admission.ts` | Pure runner plan-admission gate: rejects empty/missing-section/no-todos/unapproved plans with typed codes; accepts on `Status: Approved|Ready|Accepted`. |
...
Beyond the Phase 1 foundations, the ABG reference alignment work added runtime seams that the built-in `#default` / `#planner` / `#executer` graphs rely on. Each is self-contained, pure where stated, and test-covered (`packages/core/src/behavior/abg-reference-parity.test.ts` is the companion regression suite; `docs/abg-reference-parity-matrix.md` is the 22-row status matrix).
...
| Executer plan-admission gate | `packages/core/src/behavior/executer-plan-admission.ts` | `evaluatePlanAdmission(planText)` is the pure gate (codes: `plan_empty`/`missing_section`/`no_todos`/`not_approved`) that the executer `admit-plan` entry node uses to reject missing/malformed/unapproved plans to a terminal node before any task delegation runs. |
...
- Commit source, docs, tests, config, lockfiles, and workflows. Do not edit generated `dist`, `build`, `target`, `coverage`, `.nx`, `.mc`, `evidence`, `temp`, or reference-repo files.
...
- `.mc/` is agent state. Externally-authored files (boulder) use `.passthrough()` schemas and atomic temp-file-then-rename writes so unknown fields survive read-modify-write round-trips.
