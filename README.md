...
`ABG.md` is the root design reference. The current runtime implements a bounded coding-agent MVP over the original scaffold: provider turns, durable SQLite/libSQL sessions (SQL-only store; JSONL as archive export payload), approval-gated tools, replay projections, graph coordination, behavior/action graph execution for authorable graphs, CLI chat, desktop inspection, core desktop command services, and a versioned sidecar handshake.
...
Session navigation stays on the durable SQLite/libSQL session surface: `/new [session-id]` starts a new durable session, `/session <session-id>` switches to an existing durable session, `/sessions` lists durable sessions, `/tree` shows the durable session tree and active leaf, `/branch <entry-id>` selects an existing branch leaf, `/branch <message-id> <prompt>` continues from a parent message in a new branch, `/fork <entry-id> [session-id]` forks from a tree entry into a new durable session, and `/clone [session-id]` clones the current durable session into a fresh one. JSONL remains an archive export payload format only. `/compact` summarizes older session history into a durable compaction boundary event, keeping the session durable while reducing replay context. `/session` with no argument opens a searchable picker of sessions previously opened in the current project (selecting one attaches to it). `/resume` attaches to the most recent session for this project without starting work: it restores the transcript, projects ABG graph state when present, and shows a sticky resumable banner when work can continue, but it never auto-starts a run. `/continue` is not a session switch; it resumes resumable work on the attached session, including approval-blocked runs and interrupted runs with a durable graph checkpoint (`GraphCheckpoint` cursor and blackboard). An interrupt without a usable checkpoint prints an explicit non-resume message. A fresh prompt after attach starts new work and does not seed the prior checkpoint.
...
- The child runs under its own identity: the agent body becomes the child system prompt (the parent persona is never injected), and the spawned graph injects that prompt into its `llm-actor` node. The `yield` tool is the preferred result channel (captured via its callback); a child that finishes with a final prose answer instead has that text treated as its result, so a missing `yield` settles as a degraded-but-completed outcome (carrying a `yield_missing` settlement kind for observability), not a failure. Only a child graph that genuinely fails surfaces a child failure.
...
- Runtime startup opens only the unified database; it does not probe or automatically import prior SQL stores. The session store is SQL-only: opens do not auto-discover `sessions/*.jsonl`, and `.mc/runs/*.json` files are never auto-imported. Archive import writes SQL directly; JSONL remains an archive export payload format only.
...
- Optional on-disk JSONL logs may still live at `sessions/<session-id>.jsonl` for export convenience; they are not auto-imported on open.
...
- The SQLite/libSQL session data model, table responsibilities, indexes, archive import, and explicit export behavior are documented in [`docs/session-data-model.md`](docs/session-data-model.md).
...
- subagent orchestration via the task tool is implemented: `task` delegates a bounded sub-task to a child coding agent whose tool surface is structurally restricted (nested `task`/`job` is depth-gated; `workflow`/`team` always drop; network and `mcp__*` stay dropped except for the allowlisted `librarian`, `deep`, `reasoner`, `oracle`, `designer`, and `planner` categories).
...
- `blocked_on_approval` means the run paused for an approval decision and can be resumed with `/continue` in interactive mode or by recording an approval decision in the durable session. An `interrupted` run with a durable graph checkpoint can also be resumed with `/continue`; `/resume` only attaches to a session and never starts work by itself. Neither command claims mid-token resume or auto-run on attach.
...
- SQLite/libSQL session storage appends durable protocol events and derives replay projections, graph snapshots, approval state, branch summaries, and ABG timelines. The session store is SQL-only; JSONL remains an archive export payload format only.
...
- Durable replay: the local SQLite/libSQL `session_events` ledger reconstructs chat, approval, graph, diff, and command output state after restart; JSONL remains an archive export payload format only. Blocking input and foreground subagent waits are mirrored into the public data-dir session DB.
