...
| MCP config schema | `src/mcp-config.ts` | `McpConfigEntrySchema` (local/remote discriminated union), `MissionControlConfigSchema` (mcp + mcp_env_allowlist + lsp placeholder), `McpProjectConfigSchema` (.mcp.json mcpServers). Profile config files (`mission-control.<profile>.json[c]`, `config.<profile>.json[c]`) reuse the SAME strict `MissionControlConfigSchema`; there is no separate profile schema, only a different filename. |
