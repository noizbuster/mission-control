...
describe('abg overlay state', () => {
    ...
    describe('extractBudgetPayload', () => {
        it('returns undefined for non-budget event types', () => {
            const event: AgentEvent = {
                type: 'log',
                timestamp: TS,
            } as AgentEvent;
            expect(extractBudgetPayload(event)).toBeUndefined();
        });

        it('returns undefined when abg.emit.payload is missing', () => {
            const event: AgentEvent = {
        ...
        it('returns undefined when payload fields are non-numeric', () => {
            const event: AgentEvent = {
                type: 'policy.budget.accumulated',
                timestamp: TS,
                abg: {
                    ...
                    nodeId: 'n1',
                    signalType: 'emit',
                    emit: { type: 'policy.budget.accumulated', payload: { cents: 'no' } },
                },
            } as AgentEvent;
            expect(extractBudgetPayload(event)).toBeUndefined();
        });

        it('extracts payload fields for budget accumulated event', () => {
            const event: AgentEvent = {
                type: 'policy.budget.accumulated',
                timestamp: TS,
                abg: {
            ...
            const result = extractBudgetPayload(event);
        ...
        it('includes budgetCents when present', () => {
            const event: AgentEvent = {
                type: 'policy.budget.accumulated',
                timestamp: TS,
                abg: {
    ...
    describe('projectAgentEvent budget accumulation (v2)', () => {
        it('updates costCents from policy.budget.accumulated event', () => {
            const store = createAbgOverlayStore();
            const event: AgentEvent = {
                type: 'policy.budget.accumulated',
                timestamp: TS,
            ...
            store.update((draft) => {
                const patch = projectAgentEvent(store.getSnapshot(), event);
                Object.assign(draft, patch);
            });
            const snap = store.getSnapshot();
            expect(snap.costCents).toBe(4321);
            expect(snap.inputTokens).toBe(2000);
            expect(snap.outputTokens).toBe(1000);
            expect(snap.modelCalls).toBe(3);
        });

        it('replaces prior totals on subsequent accumulated events (not additive)', () => {
            const store = createAbgOverlayStore();
            const baseEvent: AgentEvent = {
                ...
                abg: {
                    ...
                    emit: {
                        type: 'policy.budget.accumulated',
                        payload: { cents: 100, inputTokens: 10, outputTokens: 5, modelCalls: 1 },
                    },
                },
            } as AgentEvent;
            store.update((draft) => {
                Object.assign(draft, projectAgentEvent(store.getSnapshot(), baseEvent));
            });
            const nextEvent: AgentEvent = {
                ...
                abg: {
                    ...
                    emit: {
                        type: 'policy.budget.accumulated',
                        payload: { cents: 250, inputTokens: 25, outputTokens: 12, modelCalls: 2 },
                    },
                },
            } as AgentEvent;
            store.update((draft) => {
                Object.assign(draft, projectAgentEvent(store.getSnapshot(), nextEvent));
            });
            const snap = store.getSnapshot();
            expect(snap.costCents).toBe(250);
            expect(snap.inputTokens).toBe(25);
            expect(snap.modelCalls).toBe(2);
        });
    });

    describe('extractBlackboardMutation', () => {
        it('returns undefined for non-blackboard event types', () => {
            const event: AgentEvent = { type: 'log', timestamp: TS } as AgentEvent;
            expect(extractBlackboardMutation(event)).toBeUndefined();
        });

        it('parses blackboard.set payload', () => {
            const event: AgentEvent = {
                type: 'blackboard.set',
                timestamp: TS,
                abg: {
                    ...
                    emit: {
                        type: 'blackboard.set',
                        payload: { key: 'goal_1', value: 'ship v2 features' },
                    },
                },
            } as AgentEvent;
            const result = extractBlackboardMutation(event);
            expect(result).toEqual({ kind: 'blackboard.set', key: 'goal_1', value: 'ship v2 features' });
        });

        it('parses blackboard.delete payload', () => {
            const event: AgentEvent = {
                ...
                abg: {
                    ...
                    nodeId: '$blackboard',
                    signalType: 'emit',
                    emit: { type: 'blackboard.delete', payload: { key: 'hypothesis_3' } },
                },
            } as AgentEvent;
            const result = extractBlackboardMutation(event);
            expect(result).toEqual({ kind: 'blackboard.delete', key: 'hypothesis_3' });
        });

        it('rejects payload without a string key', () => {
            const event: AgentEvent = {
                type: 'blackboard.set',
                timestamp: TS,
                abg: {
                    ...
                    nodeId: '$blackboard',
                    signalType: 'emit',
                    emit: { type: 'blackboard.set', payload: { key: 42 } },
                },
            } as AgentEvent;
            expect(extractBlackboardMutation(event)).toBeUndefined();
        });
    ...
    describe('projectAgentEvent blackboard mutations (v2)', () => {
        it('adds entry on blackboard.set', () => {
            const store = createAbgOverlayStore();
            const event: AgentEvent = {
                type: 'blackboard.set',
                timestamp: TS,
                abg: {
                    ...
                    nodeId: '$blackboard',
                    signalType: 'emit',
                    emit: { type: 'blackboard.set', payload: { key: 'observation_1', value: 'cost too high' } },
                },
            } as AgentEvent;
            store.update((draft) => {
                Object.assign(draft, projectAgentEvent(store.getSnapshot(), event));
            });
            expect(store.getSnapshot().blackboardEntries.get('observation_1')).toBe('cost too high');
        });

        it('removes entry on blackboard.delete', () => {
            const store = createAbgOverlayStore();
            store.update((draft) => {
                draft.blackboardEntries.set('goal_1', 'done');
                draft.blackboardEntries.set('goal_2', 'pending');
            });
            const event: AgentEvent = {
                type: 'blackboard.delete',
                timestamp: TS,
                abg: {
                    ...
                    nodeId: '$blackboard',
                    signalType: 'emit',
                    emit: { type: 'blackboard.delete', payload: { key: 'goal_1' } },
                },
            } as AgentEvent;
            store.update((draft) => {
                Object.assign(draft, projectAgentEvent(store.getSnapshot(), event));
            });
            expect(store.getSnapshot().blackboardEntries.has('goal_1')).toBe(false);
            expect(store.getSnapshot().blackboardEntries.has('goal_2')).toBe(true);
        });
    });

    describe('multi-graph tracking (v3-1)', () => {
        it('auto-sets focusedGraphId on first graph signal', () => {
            const store = createAbgOverlayStore();
            ...
            store.update((draft) => {
                Object.assign(draft, projectAbgSignal(store.getSnapshot(), signal));
            });
            expect(store.getSnapshot().focusedGraphId).toBe('parent-graph');
        });

        it('tracks graph summary in the registry', () => {
            const store = createAbgOverlayStore();
            const signal: AbgSignal = {
                type: 'started',
                nodeId: 'n1',
            ...
            const summary = store.getSnapshot().graphs.get('parent-graph');
            expect(summary).toBeDefined();
            expect(summary?.graphId).toBe('parent-graph');
            expect(summary?.eventCount).toBeGreaterThan(0);
        });

        it('updates graph status from graph.completed event', () => {
            const store = createAbgOverlayStore();
            store.update((draft) => {
                draft.activeGraphId = 'g1';
                draft.graphs.set('g1', {
            ...
            const event: AgentEvent = {
                ...
                timestamp: TS,
                abg: { graphId: 'g1', nodeId: 'n1', signalType: 'success' },
            } as AgentEvent;
            store.update((draft) => {
                Object.assign(draft, projectAgentEvent(store.getSnapshot(), event));
            });
            expect(store.getSnapshot().graphs.get('g1')?.status).toBe('completed');
        });

        it('captures parentGraphId when a second graph appears mid-run', () => {
            const store = createAbgOverlayStore();
            const parentSignal: AbgSignal = {
                type: 'started',
                nodeId: 'n1',
                graphId: 'parent',
            } as AbgSignal;
            store.update((draft) => {
                Object.assign(draft, projectAbgSignal(store.getSnapshot(), parentSignal));
            });
            const childSignal: AbgSignal = {
                type: 'started',
                nodeId: 'n1',
                graphId: 'child',
            } as AbgSignal;
            store.update((draft) => {
                Object.assign(draft, projectAbgSignal(store.getSnapshot(), childSignal));
            });
