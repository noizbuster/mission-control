...
const trustedProjectTrustStore = {
    getDecision: async (workspaceRoot: string) => ({
...
async function makeOptions(): Promise<EvalToolOptions & { workspaceRoot: string }> {
    ...
    return { workspaceRoot, projectTrustStore: trustedProjectTrustStore, requestPermission: allowEvalPermission };
}
...
function allowEvalPermission(request: PermissionRequest): PermissionDecision {
    return { requestId: request.id, status: 'allow', reason: 'explicit eval unit-test approval' };
}
