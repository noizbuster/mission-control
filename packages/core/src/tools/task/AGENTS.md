...
| Runtime authority | `../../agents/task-tool-runtime-authority.ts`, `../../agents/child-tool-permissions.ts` | `buildChildToolSurface` intersects tool allowlists, applies hard drops, adds `yield`, and installs category plus derived `pathPolicies` checks for every invocation resource. |
...
- Child permissions contain category rules plus a trailing nested-subagent deny. Parent agent `pathPolicies` are enforced independently by `ConcreteTaskToolRuntime`, where inherited denies override child allows. No category can re-enable nested `task` calls through policy alone.
...
- `buildChildToolSurface` always drops `task` and `job`, hard-drops `subagent`/`workflow`/`network`/`team`, and adds `yield`. Category rules and child-plus-parent-deny `pathPolicies` can filter universally denied tools up front and are enforced again against concrete invocation resources. Destructive tools are policy-controlled rather than hard-dropped.
...
- Session resume uses `task_id`. The runtime requires an idle or parked child owned by the same parent and recomputes its SHA-256 `authorityFingerprint` digest. It resumes only when that digest matches the stored authority; an unknown id or mismatch raises a non-retryable `ToolExecutionError`.

## Tests

- `task-tool.test.ts` covers every routing path, the `category` XOR `subagent_type` XOR `agent` constraint, child permission derivation, the single-spawn happy path, four-item batch waves with per-item failure isolation, background handle return, resume of known and unknown sessions, and `toModelOutput` formatting for batch, running, failed, and completed results.

## Anti-Patterns

- Do not remove the nested-subagent deny appended by task routing, the registry-layer `task` omission, hard drops, or invocation policy. They are independent child-authority guards.
