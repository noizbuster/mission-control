...
describe('vision tools (look_at + inspect_image)', () => {
    ...
    function lookAtOptions(overrides: Omit<LookAtToolOptions, 'workspaceRoot' | 'requestPermission'> = {}) {
    ...
    describe('createLookAtToolRegistration', () => {
        it('produces a valid ToolRegistration with the look_at identity', () => {
            const registration = createLookAtToolRegistration(lookAtOptions());
        ...
        it('produces a stable version hash so repeated registrations agree', () => {
            const first = createLookAtToolRegistration(lookAtOptions());
            const registry = new ToolRegistry();
            const firstAd = registry.register(first);
            const secondAd = new ToolRegistry().register(createLookAtToolRegistration(lookAtOptions()));
...
function allowPermission(request: PermissionRequest): PermissionDecision {
    return { requestId: request.id, status: 'allow', reason: 'vision test allow' };
}
