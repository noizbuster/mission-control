 ...
 * no external product names are referenced in prompts.
 *
 ...
 *   }
 *
 * Behavioral discipline (reference only; no external code imported):
 ...
 *     changes, command output), not just the child's "done" claim.
...
export const FIXER_WORKFLOW_MAX_NODE_RUNS = 128;

/**
 * The supervisor's bounded retry budget. After this many consecutive delegation failures
 * the supervisor escalates instead of retrying. Mirrors the orchestrator discipline of
 * STOP, REVERT, DOCUMENT, CONSULT, ASK USER after three consecutive failures.
 */
export const FIXER_WORKFLOW_STRIKE_BUDGET = 3;

export type FixerWorkflowGraphOptions = {
    /**
...
export function createFixerWorkflowGraph(options: FixerWorkflowGraphOptions = {}): AbgGraphSpec {
    return {
        id: FIXER_WORKFLOW_GRAPH_ID,
        version: '0.1.0',
        entryNodeId: 'intent-gate',
        defaults: {
            ...(options.model !== undefined ? { model: options.model } : {}),
            maxNodeRuns: options.maxNodeRuns ?? FIXER_WORKFLOW_MAX_NODE_RUNS,
        },
        nodes: [
            {
                id: 'intent-gate',
                ...
                config: {
                    systemPrompt:
                        "You are the intent gate for the fixer workflow. Map the user's surface request to its true " +
            ...
            },
            {
                ...
                config: { maxAttempts: FIXER_WORKFLOW_STRIKE_BUDGET, strikeBudget: FIXER_WORKFLOW_STRIKE_BUDGET },
            },
            {
                id: 'final-respond',
                kind: 'llm',
                ...
                config: {
                    systemPrompt:
                        'Synthesize the final answer from verified results already in context. ' +
